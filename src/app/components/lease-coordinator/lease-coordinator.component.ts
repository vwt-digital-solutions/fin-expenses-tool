...
@Component({
  ...
  templateUrl: './lease-coordinator.component.html',
  styleUrls: ['./lease-coordinator.component.scss']
})
...
export class LeaseCoordinatorComponent {
  ...
  constructor(
    ...
    this.columnDefs = [
      {
        headerName: 'Lease Coördinator overzicht',
        children: [
          {
  ...
  onGridReady(params: any) {
    ...
    // @ts-ignore
    this.expenses.getLeaseCoordinatorExpenses().subscribe((data) => this.rowData = [...data]);
  }

  onBtExport() {
    const params1 = {
      allColumns: true,
      processCellCallback: LeaseCoordinatorComponent.processExcelCellCallback.bind(this)
    };
